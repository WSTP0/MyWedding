...
// out += temp.substring(12,14) + temp.substring(15,17);
    ...
    public static long getDiffMinute(Date date1, Date date2) {
        ...
        long nm = 1000 * 60;
        try {
            diff = date2.getTime() - date1.getTime() % nd % nh / nm;
        } catch (Exception e) {
        }
        return diff;
    }
...
// subYear==0,说明是同一年
    ...
    /**
     * 上月最后一天(yyyyMMdd 23:59:59)
     *
     * @return
     */
    public static Date getPreMonthLastTime() {
        Calendar lastDate = Calendar.getInstance();
        lastDate.set(Calendar.DATE, 1);// 设为当前月的1号
        lastDate.add(Calendar.DATE, -1);
        return getMaxTime(lastDate.getTime());
    }
     ...
     * 将时间转换为时间戳
     */
    public static Long dateToStamp(String s,String sdf) throws ParseException{
        String res;
     ...
     * 将时间戳转换为时间
     */
    public static String stampToDate(Long s,String sdf){
        String res;
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat(sdf);
        Date date = new Date(s);
    ...
    public static long getDateDiff2(String startDate, String endDate) {
        ...
        try {
            ...
            num = (date1.getTime() - date2.getTime()) >0 ? (date1.getTime() - date2.getTime())
                    % (24 * 60 * 60 * 1000)
                    : (date2.getTime() - date1.getTime()) % (24 * 60 * 60 * 1000);
            if(num >0){
                diff = diff+1;
    ...
    public static void main(String args[]){
        System.out.println(getDateDiff2("2019-03-15 16:01:48","2019-03-15 17:01:48"));
    }
